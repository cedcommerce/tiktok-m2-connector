...
class ApiEndPoint extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    const URLS = [
        ...
        'ARISE' => [
            'live' => [
                'AUTHENTICATION' => 'https://api-backend.cifapps.com/apiconnect/request/auth?sAppId=195',
                'REFRESH_TOKEN' => 'https://app-backend.cifapps.com/magentohome/request/getNewRefershToken',
                'WEB_HOOK_URL' => 'https://5a5q2wnjx7.execute-api.us-east-2.amazonaws.com/v1/magento_webhook_py'
            ],
            'sandbox' => [
                'AUTHENTICATION' => 'https://dev.common-remote.cedcommerce.com/apiconnect/request/auth?sAppId=2',
                'REFRESH_TOKEN' => 'https://dev.common-remote.cedcommerce.com/magentohome/request/getNewRefershToken',
            ...
            'local' => [
                'AUTHENTICATION' => 'http://remote.local.cedcommerce.com:8080/apiconnect/request/auth?sAppId=30',
                'REFRESH_TOKEN' => 'http://remote.local.cedcommerce.com:8080/magentohome/request/getNewRefershToken',
                'WEB_HOOK_URL' => 'https://cr0gdi1hs5.execute-api.ap-southeast-1.amazonaws.com/v1/magento_webhook_py'
            ]
        ],
    ...
    public function sendTokenByCurl($data)
    {
        ...
        if($this->config->getShopId()) {
            $data['shop_id'] = $this->config->getShopId(); //this for arise
        }
        $finaleData = $data;
    ...
    public function productChangeSend($token, $data)
    {
        ...
        if($this->config->getShopId()) {
            $finaleData['shop_id'] = $this->config->getShopId(); //this for arise
        }
    ...
    public function orderStatusChange($token, $data)
    {
        $data['store_url'] = $this->config->getStoreurl();
        $finaleData = [];
        $finaleData["action"] = "webhook_manage";
        ...
        $this->logger->logger(
            'Order Status Change',
            'OrderS tatus Change Request data',
            json_encode($finaleData),
            'Response Data'.json_encode($response)
        );
        curl_close($ch);
